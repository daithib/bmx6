...
int process_description_tlv_ref(struct rx_frame_iterator *it)
{
	...
	assertion(-501626, ( TEST_FUNCTION(resolve_desc_extensions))); //calling:
...
//	char *vf_error = "???";
	...
	it = it_init;
		...
		assertion(-501660, (vf_data_len > 0 && vf_type <= BMX_DSC_TLV_MAX && vf_relevant <= 1 ));
...
static int32_t ref_status_creator(struct status_handl *handl, void *data)
{
        ...
        struct ref_status *status = ((struct ref_status*) (handl->data = debugReallocReset(handl->data, max_size, -300587)));
