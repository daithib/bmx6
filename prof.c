...
struct prof_ctx *prof_init( char *name, char *parent, struct orig_node *orig, struct neigh_node *neigh)
{
	...
	assertion(-500000, XOR(orig, neigh));
...
{
	if (!p || (p->before && p->active_childs == childs && prof_check(p->parent, 1) == SUCCESS))
		return SUCCESS;
...
{
	assertion(-500000, (!p->before));
	assertion(-500000, (!p->active_childs));
	p->before = (TIME_T)clock();
...
void prof_end( struct prof_ctx *p)
{
	...
	assertion(-500000, (after >= p->before));
	ASSERTION(-500000, (prof_check(p, 1) == SUCCESS));

	p->total = (after - p->before);
...
static const struct field_format prof_status_format[] = {
        ...
        FIELD_FORMAT_INIT(FIELD_TYPE_UINT,              prof_status, percent,       1, FIELD_RELEVANCE_HIGH),
        FIELD_FORMAT_END
};
